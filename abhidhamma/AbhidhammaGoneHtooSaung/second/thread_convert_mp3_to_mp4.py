...
def one(mp3s_count):
    ...
    images = [mp3 for mp3  in sorted(glob.glob("images/first/new_images/*.jpg"), key=natural_keys)]
    convert_manager = ConvertManager(mp3s[:mp3s_count], len(images), 'images/first/new_images/', threads)
...
def two(mp3s_count):
    ...
    images = [mp3 for mp3  in sorted(glob.glob("images/second/new_images/*.jpg"), key=natural_keys)]
    convert_manager = ConvertManager(mp3s[:mp3s_count], len(images), 'images/second/new_images/', threads)
    convert_manager.begin_convert()
...
def three(mp3s_count):
    ...
    images = [mp3 for mp3  in sorted(glob.glob("images/third/new_images/*.jpg"), key=natural_keys)]
    convert_manager = ConvertManager(mp3s[:mp3s_count], len(images), 'images/third/new_images/', threads)
    convert_manager.begin_convert() 
